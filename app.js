...
app.post('/register', async ({body},res)=>{
  const {nombre, apellido, game, contra, repcontra, email} = body;

  connection.query('INSERT INTO registros SET ?',{nombre, apellido, game, contra, repcontra, email}, async(error, results)=>{
    if(error){
      console.log(error);
    }else{
      res.render('register',{
